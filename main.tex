...
An isolated point $x \in X$: $\exists r > 0: B(x, r) \cap A = \{x\}$
\end{defn}
...
\end{equation*}
We know that a point $x$ is either an adherent point or an isolated point, so:
\begin{equation*}
...
\end{equation*}
The expression simplifies to the point being isolated or a limit point.
\end{proof}
...
\end{proof}
We can show that a limit point is NOT an isolated point but is either a boundary point or interior point (considering an $A$-valued sequence). Thus if we have some point in the closure, it is either a boundary or interior point (by definition). To ensure its a limit point, we use AC to find a convergent sequence.\\

On the converse, we know that a limit point is an interior or boundary point (note this is XOR not logical OR), so it must be in the closure by definition. (In essence it must be adherent but not isolated)\\

So (informally) the intuition is that when a set contains all its limit points, it contains its interior and boundary points thus its closed. However, to define a limit point, we require a convergent sequence.
