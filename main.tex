...
\end{cor}
With this corollary, we are able to define the notion of integer powers (as integers have additive inverses not accounted for by natural powers). Now we define rational powers:
\begin{cor}
$\forall p, q \in \mathbb{Z}: a^{p/q} = (a^p)^{1/q} = (a^{1/q})^p$
\end{cor}
...
\end{lem}
\begin{proof}
If $\frac{m}{n} = \frac{p}{q}$, then we know that $mq = np$ where the two are integers. We proceed by casework:
\begin{enumerate}
    \item $m, n \geqslant 0: m, n \in \mathbb{N} \land a^{mp} = (a^m)^p \land a^{nq} = (a^n)^q$ by field multiplication properties
    \item $m < 0, n \geqslant 0$, then $\exists k \in \mathbb{N}: k + m = 0$, so $a^{mp} = a^{-k \cdot p} = a^{-(kp)} = (a^{-1})^{kp} = ((a^{-1})^k)^p \land a^{nq} = (a^n)^q$
    \item The case $m \geqslant 0, n < 0$ is symmetric
    \item $m, n < 0$ is the same as the first case as the negative times negative is positive (this can be shown from field properties)
\end{enumerate}
In cases 1/4, we once again use multiplicative inverses:
\begin{equation*}
    (a^m)^p = (a^n)^q \implies ((a^m)^p)^{-1} \cdot (a^m)^p = ((a^m)^p)^{-1} \cdot (a^n)^q
\end{equation*}
We apply corollary 5.1 to get:
\begin{equation*}
    1 = (a^m)^{-p} \cdot (a^n)^q = (a^{-m})^ p \cdot (a^n)^q = ((a^{-1})^m)^p \cdot (a^n)^q
\end{equation*}
\end{proof}
...
\end{lem}
\begin{proof}
We let $r = \frac{m}{n}$ and $s = \frac{p}{q}$, where $m, n, p, q \in \mathbb{Z} \land p, q \neq 0$, then $r + s = \frac{m}{n} + \frac{p}{q} = \frac{mq + np}{pq}$. Thus:
\begin{equation*}
    a^{r+s} = a^{(mq + np)/pq} = (a^{1/pq})^{mq + np}
\end{equation*}
Now $mq + np$ is an integer expression, by field properties, we can rewrite the expression as:
\begin{equation*}
...
\begin{equation*}
    a^{\tilde{x}+y} = \sup\{a^z: z \in \mathbb{Q} \land z \leqslant \tilde{x} + y\}
\end{equation*}
Since $a^y = \sup\{a^p: p \in \mathbb{Q} \land p \leqslant y\}$ and $a^x = \sup\{a^q: q \in \mathbb{Q} \land q \leqslant x\}$:
\begin{equation*}
    a^x \cdot a^y = \sup\{a^p\} \cdot \sup\{a^q\}
\end{equation*}
Multiplying the supremums combines the predicates in a logical and. By ordered field properties:
\begin{equation*}
    q \leqslant x \land p \leqslant y \implies q + p \leqslant x + y
\end{equation*}
Thus $a^x \cdot a^y \leqslant \sup\{a^{q+p}: q+ p \leqslant x + y\} = a^{x+y}$. We now prove the opposite inequality, that $a^{x+y} \leqslant a^x \cdot a^y$. We choose a fixed $\tilde{q} \leqslant x$ and a fixed $\tilde{p} \leqslant y$, then:
\begin{equation*}
    a^{x+y} = \sup\{a^{p + \tilde{q}}: p + \tilde{q} \leqslant x + y\}
\end{equation*}
\begin{equation*}
    p + \tilde{q} \leqslant x + y \implies p \leqslant x + y - \tilde{q}
\end{equation*}
By definition of supremum, we get that $y \leqslant x + y - \tilde{q} \implies \tilde{q} \leqslant x$. Similarly if we fix a $\tilde{p}$ such that:
\begin{equation*}
    a^{x+y} = \sup\{a^{\tilde{p} + q}: \tilde{p} + q \leqslant x + y\}
\end{equation*}
\begin{equation*}
    \tilde{p} + q \leqslant x + y \implies q \leqslant x + y - \tilde{p}
\end{equation*}
By supremum definition, we get $x \leqslant x + y - \tilde{p} \implies \tilde{p} \leqslant x$. Thus for any arbitary $p$, $q$:
\begin{equation*}
    p + q \leqslant x + y \implies p < x \land q < y
\end{equation*}
Thus $a^{x+y} \leqslant a^x \cdot a^y$, proving equality.
