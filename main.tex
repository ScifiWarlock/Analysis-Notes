...
\text{NOTE}: $(P \implies Q)$ is not the same as $\lnot P \implies \lnot Q$. It is equivalent to $Q \implies P$ by Lemma 1.2. We see the truth table below and compare it to the one above:
\begin{center}
\begin{tabular}{| c | c  | c |}
\hline
...
\end{equation*}
By the total ordering of $F$ and the fact that each field has its copy of the rationals, we can find the set of rationals strictly less than $x$. This is the associated Dedekind cut $A$.
\end{proof}
...
\end{equation*}
We can construct an injection $f: \{0, 1\}^\mathbb{N} \times \{0. 1\}^\mathbb{N} \xrightarrow{} \{0, 1\}^\mathbb{N}$. If we take a pair of sequences, simply making the elements from the first sequence as odd elements and from the second as even elements, we get a unique sequence. Therefore:
\begin{equation*}
...
\section{Topology}
\subsection{Open and Closed sets (In Metric Spaces)}
...
Let us provide some motivation behind the notion of a topology. By Definition 9.2, a set is closed if its complement is open. How do we define an open set? Perhaps an open set is one whose complement is closed. We arrive at a circular definition.\\

Let's try a different approach. A closed set is one which has its boundary in it, and an open set is one whose complement is closed. How do we define the boundary? Perhaps we can define it as the set without its inside (interior). How do we define the interior of a set?\\

Perhaps (and this is not formal) we can call it the largest open proper subset of the closed set. How do we define this maximal subset? It is maximal in relation to other open subsets, so we need a general definition of an open set.
\pagebreak
\begin{defn}
    A topology $\uptau$ is a collection of subsets of a space $X$ which are open if:
    \begin{enumerate}
        \item $\varnothing, X \in \uptau$
        \item $\bigcup A \in \uptau$ (Closed under Arbitrary Unions - finite/infinite)
        \item $\bigcap A \in \uptau$ (Closed under \textbf{finite} intersection)
    \end{enumerate}
\end{defn}
If all these requirements are satisfied, then the collection of subsets is called a topology $\uptau$ on $X$. The "coarsest" topology is simply $\{\varnothing, X\}$, which can be checked follows all definitions.

\medskip
\begin{defn}
    A topology can also be defined as a collection of closed sets such that:
    \begin{enumerate}
        \item Since $X = \varnothing^c$, and since $X$ is open, then $\varnothing$ is closed. Similarly since $\varnothing$ is open, then $X$ is closed.
        \item $(\bigcup A)^c = \bigcap A^c$. $A^c$ is closed, since $A$ is open, and similarly $(\bigcup A)^c$ is closed. Therefore, the arbitrary intersection of closed sets belongs in the topology.
        \item Similarly $(\bigcap A)^c = \bigcup A^c$, so the finite union of closed sets belongs to the topology.
    \end{enumerate}
\end{defn}

Building upon the notion of topology, we can often "define" or "generate" a topology through a smaller collection of subsets of $X$, which we call the basis.
\begin{defn}
    A basis $\mathcal{B}$ of a topology $\uptau$ is a collection of $X$:
    \begin{enumerate}
        \item $\forall x \in X, \exists B \in \mathcal{B}: x \in B \land B \subseteq X$
        \item $\forall B_1, B_2 \in \mathcal{B}, \forall x \in B_1 \cap B_2, \exists B_3 \in \mathcal{B}: x \in B_3 \subseteq B_1 \cap B_2$
    \end{enumerate}
\end{defn}
The first part of the definition ensures that every point in the space is "covered" by at least one basis element (if not more depending on the space). That way, we can describe a topology on $X$ adequately (all of $X$ must be in the topology).\\

We will touch on the purpose of the second definition in the following lemma:
\begin{lem}
    A topology $\uptau$ \textbf{generated} by a basis $\mathcal{B}$ is defined as:
    \begin{equation*}
        \uptau \coloneq \{U \subseteq X: (\forall x \in U, \exists B \in \mathcal{B}: x \in B \subseteq U)\}
    \end{equation*}
\end{lem}
Here we define a topology "generated" by a basis as the collection of subsets that meet the criteria of openness. While our previous criteria for openness was the topological axioms, we introduce a more simple construct of a basis to define openness.\\

We see that each basis element is trivially open by this definition as it is a subset of $X$ and a subset of itself. The reader can check that this corresponds to the definitions given in the previous section of open/closedness in a metric space. So the basis for metric spaces is the collection of all open ball subsets of $(X, \varrho)$.\\

We see that such generated topology does follow the topological axioms (so if we can come up with a collection/basis that satisfies Defn 9.5, then we fulfill the topological axioms).\\

The following lemma will talk about the topology induced by a metric on a space specifically, but the proof can be generalized to non metrizable spaces.

\pagebreak
\begin{lem}
    A metric $\varrho$ on $X$ induces a topology $\uptau \coloneq \{O \subseteq X: O \ \text{open}\}$
\end{lem}
For the first statement, we see that $\varnothing$ is trivially open as we can always define open balls that are subsets of it since it contains no elements. $X$ is also trivially open as any open ball must be a subset of it (there exists nothing beyond $X$).\\

Given a collection of open subsets $A$, then:
\begin{equation*}
    x \in \bigcup A \implies \exists A_i \in A: x \in A_i \land A_i \ \text{open}
\end{equation*}
So since $A_i$ is open, we can find an open ball $B(x, r) \subseteq A_i$, so $\forall x \in \bigcup A, \exists B(x, r) \subseteq A_i \subseteq \bigcup A$. Thus the union is open.\\

Now given the intersection $\bigcap A$, then:
\begin{equation*}
    \forall x \in \bigcap A, \forall A_i \in A: x \in A_i
\end{equation*}
Since each $A_i$ is open, then we can find an open ball $B(x, r_i)$ for each subset such that $B(x, r_i) \subseteq A_i$. We see that the balls are nested:
\begin{equation*}
    B(x, r_0) \subseteq B(x, r_1) \subseteq ... B(x, r_i)
\end{equation*}
If $r_0 \coloneq \min\{r_i\}$, then $\forall A_i: B(x, r_0) \subseteq A_i$, so for any point in the intersection, we can find an open ball contained in the intersection, meaning the intersection is open.\\

To generalize this part, we know that $\forall x \in A_i, \exists B(x, r_i)$. Now this is where the second part of the basis definition plays a role:
\begin{equation*}
    \forall x \in A_1, A_2, \exists B_1, B_2: x \in B_1 \subseteq A_1 \land x \in B_2 \subseteq A_2
\end{equation*}
\begin{equation*}
    \forall x \in B_1 \cap B_2, \exists B_3: x \in B_1 \land x \in B_2 \implies x \in A_1 \land x \in A_2 \implies x \in B_3 \subseteq B_1 \cap B_2 \subseteq A_1 \cap A_2
\end{equation*}
So for any point in the intersection, we can find a basis element (in this case open ball) that belongs in the intersection. We repeat the above process inductively (as we only deal with finite intersections), showing us that the finite intersections are also open.
e of the set relies on arbitrary intersections which is possible due to the arbitrary intersections of closed set collections in the topology.

\medskip
We can also say the following about a topology generated by a basis:
\begin{lem}
    The topology $\uptau$ generated by the basis $\mathcal{B}$ is equivalent to the collection of unions of subcollections of $\mathcal{B}$.
\end{lem}
\begin{proof}
We call the topology of unions $\uptau_B$. We assume that $\mathcal{B} \subseteq \uptau_B$. Then any arbitrary unions and finite intersections of basis elements belong in the topology. By our initial definition of $\uptau$ and the basis, $\mathcal{B} \subseteq \uptau$ and so its arbitrary unions and intersections also belong on the topology.\\
 
To show the two topologies are equivalent, we aim to show all open sets in $\uptau$ are unions of basis elements. In essence, both the topologies are equally fine as the arbitrary unions and finite intersections of $\mathcal{B}$ would be equivalent representations of the existing sets in $\uptau$ defined by the first definition.\\

We know by the definition of $U \in \uptau$:
\begin{equation*}
    \forall x, \forall B_x \subseteq U: x \in U \implies \exists B_x: x \in B_x
\end{equation*}
\begin{equation*}
    \forall x: x \in U \implies x \in \bigcup B_x \iff U \subseteq \bigcup B_x
\end{equation*}
We do not require Axiom of Choice here as the property of bases allows us to generate a (not necessarily unique) collection of basis elements.\\

Now we want to prove $\bigcup B_x \subseteq U$. We can either use the collection of all basis elements that are subsets of $U$ (this does not require choice as we have a rule), so if $x$ is in its union, then $x$ is one of the subsets which means $x$ is in $U$.\\

Alternatively, we can select one such collection where each $x \in U$ belongs to one specific basis element which is a subset of $U$. Then:
\begin{equation*}
    x \in \bigcup B_x \implies \exists B_x \in \mathcal{B}: x \in B_x \implies x \in U
\end{equation*}
Thus $\bigcup B_x \subseteq U$, thus $U = \bigcup B_x$, showing that the two topologies $\uptau_B$ and $\uptau$ are equivalent. (To summarize, we defined the first topology $\uptau_b$ and then showed that any element in $\uptau$ has an equivalent representation in $\uptau_B$).\\

The reason why the second approach required axiom of choice is because while we can guarantee a basis element which is a subset of $U$ containing $x$ for every $x \in U$, we cannot guarantee ONE, UNIQUE element (there can be multiple). Thus choosing one unique element means we are chosing a basis for each point from a possibly infinite collection of basis element families (depending on $X$, we may not need Axiom of Choice, but the proof is for full generality, so we assume it).
\end{proof}
\begin{note}
An open cover (not rigorous definition, just for intuition sake) is a collection of subsets such that their union is equal to the whole space $X$. A basis acts like an "open cover for elements of the topology". Any open set in the topology defined by the initial definition wrt bases can be represented by the union of a subcollection of $\mathcal{B}$.\\

In other words, that subcollection of basis elements covers that member of the topology so all that remains in the topology are the basis elements and their arbitrary unions and intersections.
\end{note}

\medskip
\begin{defn} We define the interior and closure:
    \begin{enumerate}
        \item The interior of a set $A$: $\text{int}(A) \coloneq \bigcup \{O \in \uptau: O \subseteq A\}$
        \item The closure of a set $A$: $\bar{A} \coloneq \bigcap \{C \in \uptau: A \subseteq C\}$
    \end{enumerate}
\end{defn}
The closure of the set relies on arbitrary intersections which is possible due to the arbitrary intersections of
closed set collections in the topology.
...
\end{lem}
\begin{proof}
If $x \in \text{int}(A)$, then:
\begin{equation*}
    \forall x: x \in \text{int}(A) \implies \exists O \subseteq A: x \in O
\end{equation*}
Since $x \in O \subseteq A$, then $x \in A$, so $\text{int}(A) \subseteq A$.\\

If $x \in A$, then:
\begin{equation*}
    \forall x, \forall C \in \uptau: x \in A \land A \subseteq C \implies x \in C
\end{equation*}
\begin{equation*}
    \implies \forall x: x \in A \implies \forall C \in \uptau: x \in C \iff x \in \bar{A}
\end{equation*}
Therefore $A \subseteq \bar{A}$, proving our lemma.
\end{proof}

\medskip
\begin{lem} The following equivalences hold:
    \begin{enumerate}
        \item $A \ \text{open} \iff A = \text{int}(A)$
        \item $A \ \text{closed} \iff A = \bar{A}$
    \end{enumerate}
\end{lem}
\begin{proof}We prove the first statement:\\
$\implies$: If $A$ is open, then $A$ belongs to the union of open sets which are subsets of $A$, thus $\text{int}(A) \subseteq A$. We know that $A \subseteq \textsl{int}(A)$, thus $A = \textsl{int}(A)$.\\

$\impliedby$: If $A = \text{int}(A)$, then $A$ is the union of open sets so $A$ is an open set.
\end{proof}

\begin{proof} The proof of the second statement is similar:\\
$\implies$: If $A$ is closed, then $A$ belongs to collection of closed sets contributing to the intersection, thus $\bar{A} \subseteq A$. We know that $A \subseteq \bar{A}$, so $A = \bar{A}$.\\

$\impliedby$: If $A = \bar{A}$, then $A$ is an arbitrary intersection of closed sets, which by topology axioms, $A$ is closed.
\end{proof}

\medskip
\begin{defn}
The boundary of a set $A$ or $\partial A$ is defined as $\bar{A} \setminus \text{int}(A)$
\end{defn}
If $A \subseteq X$, then we can write $\partial A$ as $\bar{A} \cap (X \setminus \textsl{int}(A))$. Since both $\bar{A}$ and $X \setminus \text{int}(A)$ are closed, then their intersection is closed.

\medskip
\begin{lem}
Let $(X, \varrho)$ be a matric space. Prove that for all sets $A, B \subseteq X$, the following holds:
\begin{equation*}
    A \subseteq B \implies \text{int}(A) \subseteq \text{int}(B) \land \bar{A} \subseteq \bar{B}
\end{equation*}
and
\begin{equation*}
    X \setminus \text{int}(A) = \overline{X \setminus A} \land X \setminus \bar{A} = \text{int}(X \setminus A)
\end{equation*}
In particular.
\begin{equation*}
...
\end{equation*}
\end{lem}
\begin{proof} We know that if $x \in A$, then $x \in \bar{A}$. Since $\bar{A} = \text{int}(A) \cup \partial A$, then $x \in \text{int}(A) \lor x \in \partial A$. If we consider $x \in \text{int}(A)$:
\begin{equation*}
    x \in \text{int}(A) \iff \exists O \subseteq A: x \in O
\end{equation*}
\begin{equation*}
    \implies O \subseteq A \subseteq B \implies O \subseteq \text{int}(B)
\end{equation*}
We see the last implication as $O$ is an open subset of $B$ which means it contributes to the union of open sets that make up the interior. Thus $x \in \text{int}(B)$, showing $\text{int}(A) \subseteq \text{int}(B)$.\\

If $x \in A$ (not necessarily in the interior), then $x \in \bar{A}$:
\begin{equation*}
    x \in \bar{A} \implies \forall C \subseteq X: A \subseteq C \land C \ \text{closed} \implies x \in C
\end{equation*}
Since $A \subseteq B$ and $B \subseteq \bar{B}$, then we see that $A \subseteq \bar{B}$. So $\bar{B}$ is one such set that contributes to the closure of $A$. Thus:
\begin{eqnarray}
    \forall x: x \in \bar{A} \implies x \in \bar{B} \iff \bar{A} \subseteq \bar{B}
\end{eqnarray}
We first prove the following statement:
\begin{equation*}
    A \subseteq B \implies X \setminus B \subseteq X \setminus A
\end{equation*}
If $x \in X \setminus B$ or $B^c$, then $x \notin B \implies x \notin A$ (contrapositive of inclusion), so $B^c \subseteq A^c$.\\

For the second statement, we see that by DeMorgan's Laws:
\begin{equation*}
    X \setminus \text{int}(A) = X \setminus \bigcup \{O \subseteq X: O \ \text{open} \land O \subseteq A\} = (\bigcup \{O \subseteq X: O \ \text{open} \land O \subseteq A\})^c
\end{equation*}
\begin{equation*}
    = \bigcap\{X \setminus O \subseteq X: X \setminus O \ \text{closed} \land X \setminus A \subseteq X \setminus O\}
\end{equation*}
Then by definition of closure, $X \setminus \text{int}(A) = \overline{X \setminus A}$. We now prove the statement for $X \setminus \bar{A}$:
\begin{equation*}
    X \setminus \bar{A} = X \setminus \bigcap\{C \subseteq X: C \ \text{closed} \land A \subseteq C\} = (\bigcap\{C \subseteq X: C \ \text{closed} \land A \subseteq C\})^c
\end{equation*}
\begin{equation*}
    = \bigcup\{X \setminus C \subseteq X: X \setminus C \ \text{open} \land X \setminus C \subseteq X \setminus A\} = \text{int}(X \setminus A)
\end{equation*}
Finally, by definition 9.6, we know that $\partial A = \bar{A} \cap (X \setminus \text{int}(A)) = \bar{A} \cap \overline{X \setminus A} = \overline{X \setminus A} \cap (X \setminus X \setminus \bar{A}) = \overline{X \setminus A} \cap (X \setminus \text{int}(X \setminus A)) = \partial(X \setminus A)$.\\
\end{proof}

\medskip
\subsection{More Topological Notions}
The following notions can be generalized for any topology by replacing the metric ball with any open set $U \in \uptau$. However, since we verified that the open balls are a basis for a metric space topology and that every element in the topology is the union of basis elements/open balls, then we simply use the notion of open balls since they make up the topology $\uptau_B$:\\

\begin{defn}
An adherent point $x \in X$: $\forall r > 0: B(x, r) \cap A \neq \varnothing$
\end{defn}
In essence, an adherent point is a point for which we can find a subset in the topology intersection with $A$. Since our topology is constructed by open balls and their unions and intersections, such a set is an open ball.\\

Because of the notion of distance, we can make this open ball arbitrarily small.\\

\medskip
\begin{lem}
$x \in X$ is adherent if and only if $x \in \bar{A}$
\end{lem}
\begin{proof}
$\implies$: We prove the contrapositive:
\begin{equation*}
\forall x \in X: x \notin \bar{A} \implies \exists r > 0: B(x, r) \cap A = \varnothing
\end{equation*}
If $x \notin \bar{A}$, then $x \in X \setminus \bar{A}$. We prove the contrapositive: Since the complement is open and $A \subseteq \bar{A}$:
\begin{equation*}
    \exists r > 0: B(x, r) \subseteq X \setminus \bar{A} \subseteq X \setminus A \implies B(x, r) \cap A = \varnothing
\end{equation*}
$\impliedby$: We prove by contrapositive again:
\begin{equation*}
\forall x \in X: \exists r > 0: B(x, r) \cap A = \varnothing \implies x \notin \bar{A}
\end{equation*}
If $x$ is not adherent, then $\exists r > 0: B(x, r) \cap A = \varnothing$. Now:
\begin{equation*}
    \forall x \in X \setminus \{\text{adherent points}\}, \exists r > 0: B(x, r) \cap A = \varnothing
\end{equation*}

\end{proof}
We can conclude from this lemma that the set of adherent points is the closure of $A \subseteq X$.

\pagebreak
\begin{defn}
A boundary point $x \in X$: $\forall r > 0: B(x, r) \cap A \neq \varnothing \land B(x, r) \cap (X \setminus A) \neq \varnothing$
\end{defn}
Since the boundary $\partial A \coloneq \bar{A} \setminus \text{int}(A) = \bar{A} \cap (X \setminus \text{int}(A))$. By the definition of boundary/adherent point, we see that:
\begin{equation*}
    x \ \text{boundary pt} \implies x \in \bar{A} \land x \in \overline{X \setminus A}
\end{equation*}
By Lemma 9.10, we then see that $\overline{X \setminus A} = X \setminus \text{int}(A)$, so $x \in \partial A$. So the set of boundary points makes up $\partial A$.

\medskip
\begin{defn}
An interior point $x \in X$: $\exists r > 0: B(x, r) \subseteq A$. (Question to self, what if it were $\forall r > 0$?)
\end{defn}
The set of interior points is the interior of the set $A$. This is because the open ball would belong to an open subset of $A$, so (will complete proof later)

\begin{defn}
An exterior point $x \in X$: $\exists r > 0: B(x, r) \cap A = \varnothing$
\end{defn}
The set of exterior points is the set $X \setminus \bar{A}$. (will complete proof later)

\medskip
\begin{defn}
A limit point $x \in X$: $\forall r > 0: (B(x, r) \cap A) \setminus \{x\} \neq \varnothing$.\\
An isolated point $x \in A$: $\exists r > 0: B(x, r) \cap A = \{x\}$
\end{defn}
Open balls around limit points still contain other points of $A$ besides the limit point, unlike isolated points. So isolated points are the negation of limit points (rewritten as $\exists r > 0: (B(x, r) \cap A)\setminus \{x\} = \varnothing$).\\

The idea of a limit point is similar to what was done with the $\epsilon$ proofs. We are just taking an arbitrary sized metric ball with radius $\epsilon$ or $r$, for which we must be able to find any point in that sequence (or in this case space) within that radius.\\

Furthermore by definition, an isolated point always belongs to the set $A$ (because it belongs in the intersection).

\medskip
\begin{lem}
Every isolated point is a boundary point of $A$
\end{lem}
\begin{proof}
We first tackle the easy requirement that an isolated point is adherent to $A$. Since the isolated point $x$ is defined to be in $A$:
\begin{equation*}
    \exists r > 0: B(x, r) \cap A = \{x\} \implies \forall r > 0: B(x, r) \cap A \neq \varnothing
\end{equation*}
This is because $x \in A$ is always contained in any open ball radius $r$, so the intersection of the ball and $A$ is always nonempty, thus $x$ is adherent (thus its in the closure).\\

We now prove that $\forall r > 0: B(x, r) \cap (X \setminus A) \neq \varnothing$ (it is adherent to the complement of the set). The definition of an adherent point tells us that $\exists r_0 > 0: B(x, r) \cap A = \{x\}$. So:
\begin{equation*}
    r \leqslant r_0 \implies B(x, r) \subseteq B(x, r_0)
\end{equation*}
So since $x \in A$ and $x \in B(x, r)$, then $B(x, r) \cap A = \{x\}$. Thus all other elements in the open ball belong in $X \setminus A$.\\

If $r > r_0$, then $B(x, r_0) \subseteq B(x, r)$. Since $B(x, r_0)$ contains elements in $X \setminus A$, then so does $B(x, r)$. So we have shown for any isolated point $x \in A$:
\begin{equation*}
...
\end{lem}
Since a limit point $x \in X$ is defined such that $\forall r > 0: (B(x, r) \cap A) \setminus \{x\} \neq \varnothing$, then we see that $B(x, r) \cap A \neq \varnothing$, thus it is adherent or in the closure of $A$.

\medskip
\begin{lem}
$\bar{A} = \{x \in X: \text{limit point}\} \cup \{x \in X: \text{isolated point}\}$ where the two sets in the union are disjoint.
\end{lem}
\begin{proof}
The disjointness of the two sets follows from the definitions. In Defn 9.12, we rewrote the statement of an isolated point which is the opposite of a limit point, so a point cannot be both a limit point and isolated point.
\begin{equation*}
    x \in \bar{A} \iff \forall r > 0: B(x, r) \cap A \neq \varnothing
\end{equation*}
We know that a point $x$ in the closure is either an isolated (and adherent) or adherent but not isolated, so:
\begin{equation*}
    \iff (\exists r > 0: B(x, r) \cap A = \{x\}) \lor ((\forall r > 0: B(x, r) \cap A \neq \varnothing) \land \lnot(\exists r > 0: B(x, r) \cap A = \{x\}))
\end{equation*}
\begin{equation*}
    \iff (\exists r > 0: B(x, r) \cap A = \{x\}) \lor ((\forall r > 0: B(x, r) \cap A \neq \varnothing) \land \forall r > 0: B(x, r) \cap A \neq \{x\})
\end{equation*}
The expression simplifies to the point being isolated or a limit point, a different definition for closure than interior and boundary union.
\end{proof}

\medskip
\subsection{Sequential Closedness}
\begin{thm}
$A \ \text{closed} \iff (\forall \{x_n\}_{n \in \mathbb{N}}\in A^{\mathbb{N}}, \forall x \in X: x_n \xrightarrow{} x \implies x \in A)$
\end{thm}
\begin{proof}
We prove both implications:\\
($\implies$) For a sequence $\{x_n\}$, because $A$ is closed, then $A = \bar{A}$. If $x_n \xrightarrow{} x$, then by the definition of convergent:
\begin{equation*}
    \forall r > 0, \exists n_0 \geqslant 0, \forall n \geqslant n_0: x_n \in B(x, r) 
\end{equation*}
Since $x_n \in A$, then $B(x, r) \cap A \neq \varnothing$, thus $x$ is an adherent point so $x \in A = \bar{A}$.\\

($\impliedby$) We prove by contrapositive. If $A$ is not closed, then we show that $x_n \xrightarrow{} x \land x \notin A$ is true. If $x \notin A$ then $x \in X \setminus A$ and $X \setminus A$ is not open since $A$ not closed.\\

Therefore, (informally), $X \setminus A$ has a boundary part (its "clopen"), so we see that:
\begin{equation*}
    \forall r > 0: B(x, r) \cap A \neq \varnothing
\end{equation*}
Since $\forall x_n: x_n \in A$, we need to find a sequence such that $\forall \epsilon > 0, \exists n_0 \geqslant 0, \forall n \geqslant n_0: \varrho(x_n, x) < \epsilon$. Alternatively $\forall \epsilon > 0, \exists n_0 \geqslant 0, \forall n \geqslant n_0: x_n \in B(x, \epsilon)$. (This is the same as the proposition of an open ball is $\varrho(x, x_n) < \epsilon$)\\

(\textbf{Axiom of Choice}): The sequence allows us to say that $B(x, r) \cap A \neq \varnothing$ as $\forall n \geqslant 0: x_n \in A$. To choose from an infinite family, we require the axiom of choice. We give the choice rule and because $x \notin A$:
\begin{equation*}
    \{x_n\}_{n \in \mathbb{N}} \coloneq \bigtimes_{n \in \mathbb{N}} A \cap (B(x, 2^{-n})\setminus\{x\}) = \bigtimes_{n \in \mathbb{N}}(A \cap B(x, 2^{-n}))\setminus \{x\}
\end{equation*}
Inductively we see that we create an $A$-valued sequence such that we always get that $x_n \in B(x, 2^{-n})$ or that $\varrho(x_n, x) < 2^{-n}$ ensuring convergence while $x \notin A$. We see that we use the definition of $x$ being a limit point of the set $A$ and equating it to the limit of a sequence (which under the hood invokes the Hausdorff condition).
\end{proof}
...
\begin{equation*}
    \forall x \in X: x \in \bar{A} \iff \exists \{x_n\}_{n \in \mathbb{N}} \in A^\mathbb{N}: x_n \xrightarrow{} x.
\end{equation*}
\end{cor}
\begin{proof}
($\implies$) If $x \in \bar{A}$, then it is adherent so $B(x, r) \cap A \neq \varnothing$. To choose a sequence such that the intersection is true $x_n \in B(x, r) \cap A$, we use the same process as Thm 9.1 to obtain a sequence (requires Axiom of Choice).\\

($\impliedby$) If we can find a sequence that converges to $x$, then $\forall r > 0, \exists n_0 \geqslant 0, \forall n \geqslant n_0: x_n \in B(x, r)$. Since $x_n \in A$, then $B(x, r) \cap A \neq \varnothing$, so $x$ is adherent, thus $x \in \bar{A}$.
\end{proof}
If we have some point in the closure, it is either a limit point or isolated point (by definition). To ensure its a limit point, we use AC to find a convergent sequence; additionally for isolated points, they already belong to the set by definition so they belong to the closure, so an $A$-valued sequence is the constant sequence.\\

On the converse, we know that a limit is either a limit point or isolated point (note this is XOR not logical OR), so it must be in the closure by definition.\\

\medskip
\begin{defn}
A set $B \subseteq A$ is dense in $A$ iff $\forall a \in A, \exists \{b_n\}_{n \in \mathbb{N}} \in B^\mathbb{N}: b_n \xrightarrow{} a$
\end{defn}
We see by the previous lemma that if we have a $b$-valued convergent sequence, then it must converge to something in the closure of $B$ as the limit point is either in the interior or boundary.\\

Since we already define all points in $A$ as the limit points of $b$-valued convergent sequences, then we see that all points in $A$ must belong to the closure of $B$ so $A \subseteq \bar{B}$. This is an equivalent definition for (sequentially) dense.\\

We can build on this further. If we have a metric, then we have a topology generated by the basis of open balls. Then by the definition of convergent sequences:
\begin{equation*}
    \forall r > 0, \exists n_0 \geqslant 0, \forall n \geqslant n_0: b_n \in B(a, r)
\end{equation*}
So we can always find an arbitarily close point of the sequence to $a$, so no matter how close we look (eg if we look at some interval of points in $A$ centered around $a$ then we can always find some point in $B$).

\medskip
\begin{defn}
A metric space is deemed \textbf{separable} if:
\begin{equation*}
    \exists A \subseteq X: A \ \text{countable} \land \bar{A} = X
\end{equation*}
\end{defn}
The reals $\mathbb{R}$ for example are separable as we can find a countable subset (e.g. the rationals $\mathbb{Q}$) such that their closure is the reals themselves. When we say $\bar{A} = X$, we mean that $A$ is dense in $X$.\\

This is due to our equivalent definition of density. If $A$ is dense in $X$, then $X \subseteq \bar{A}$ since for all $x \in X$, we can find an $A$-valued sequence whose limit point is $x$, thus $x \in \bar{A}$. However, since $X$ is the whole space it is not possible that $X \subset \bar{A}$, thus $X = \bar{A}$.\\

So a metric space $X$ is separable if:
\begin{equation*}
    \exists A \subseteq X: A \ \text{countable} \land A \ \text{dense}
\end{equation*}
