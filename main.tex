...
\begin{equation*}
    0 < 1 \implies \sum_{k=1}^{n-1}(1) < \sum_{k=1}^{n-1}(1) _ 1 = \sum_{k=1}^{n}(1) = 0
\end{equation*}
Now assume the inequality to be true, then by mulitplicative property of ordered fields:
\begin{equation*}
    \implies \sum_{k=1}^{n-1}(1) \cdot \sum_{k=1}^{n-1}(1) < 0
\end{equation*}
\begin{eqnarray}
    \iff (n-1)^2 = n^2 - 2n + 1 < 0
\end{eqnarray}
...
\begin{lem}
    $\forall A \subseteq \mathbb{R}_F: (\exists b \in F, \forall a \in A: a \leqslant b): \sup: A \xrightarrow{} F$ and $\sup$ is an isomorphism
\end{lem}
\noindent This lemma is essentially the restatement of the completeness of $F$. If we take any subset bounded above, then it admits a supremum. However, here we define the supremum as a map from a set to an element in the field. We prove it is an isomorphism for a cut specifically:
\begin{proof}
...
\end{equation*}
This inequality we can derive from the ordering of $F$ and the construction of the cuts. Then $b$ is an upper bound for $A$ and by definition of supremum $\sup(A) \leqslant b$.
\end{proof}
