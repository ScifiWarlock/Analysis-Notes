...
\begin{equation*}
    |a_{n+1} - a_n| \leqslant |a_{n+1}| + |a_n|
\end{equation*}
\end{proof}
...
\end{defn}
We then define a closed set as one whose complement is open.\\
...
\end{equation*}
We define open balls in $A = \bar{B}^c$ and just like Lemma 9.2 find a suitable radius. By the triangle inequality:
\begin{equation*}
...
\end{equation*}
So if we set $0 < r' < \varrho(x, y) - r$, we see that $\varrho(x, z) > r$ which means $z \in A \lor B(y, r') \subseteq A$. We also know that $\varrho(x, y) - r > 0$ by definition of $A$.
\end{document}
