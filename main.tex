...
\begin{defn}
    ...
    \end{enumerate}
\end{defn}
If all these requirements are satisfied, then the collection of subsets is called a topology $\uptau$ on $X$. The "coarsest" topology is simply $\{\varnothing, X\}$, which can be checked follows all definitions. To discuss the finest topology, we show the following lemma:

\pagebreak
\begin{lem}
    A metric $\varrho$ on $X$ induces a topology $\uptau$
\end{lem}
