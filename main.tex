...
($\impliedby$) We prove by contrapositive. If $A$ is not closed, then if $x \in A$ then $x \in \bar{A}$, so $x$ is adherent:
\begin{equation*}
    \forall r > 0: B(x, r) \cap A \neq \varnothing
\end{equation*}
...
(\textbf{Axiom of Choice}): The sequence allows us to say that $B(x, r) \cap A \neq \varnothing$ as $\forall n \geqslant 0: x_n \in A$. To choose from an infinite family, we require the axiom of choice. We give the choice rule:
\begin{equation*}
    \{x_n\}_{n \in \mathbb{N}} \coloneq \bigtimes_{n \in \mathbb{N}} A \cap (B(x, 2^{-n})\setminus\{x\})
\end{equation*}
Inductively we see that we create an $A$-valued sequence such that we always get that $x_n \in B(x, 2^{-n})$ or that $\varrho(x_n, x) < 2^{-n}$ ensuring convergence.
\end{proof}
